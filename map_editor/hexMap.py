            ...
            if new_grid > 0:
                ...
                if new_hex > 0:
                    ...
                    for col in swap_cols:
                        ...
                        old_val = self.hex_df[col].iloc[df_old_id].copy()
                        self.hex_df[col].iloc[df_old_id] = self.hex_df[col]\
                            .iloc[df_new_id].copy()
                        self.hex_df[col].iloc[df_new_id] = old_val
                    
                else:
                    # Change to new hex
                    df_new_id = self.grid_df.index[(self.grid_df.xid == new_x) & 
                                                   (self.grid_df.yid == new_y)]
                    new_wgs_x = self.grid_df.geometry.iloc[df_new_id].x
                    new_wgs_y = self.grid_df.geometry.iloc[df_new_id].y

                    new_wgs_polygon = self._draw_hex(new_wgs_x, new_wgs_y,
                                                     self.hex_width, 'wgs')

                    # Replace into table
                    self.hex_df['xid'].iloc[df_old_id] = new_x
                    self.hex_df['yid'].iloc[df_old_id] = new_y
                    self.hex_df['x'].iloc[df_old_id] = new_wgs_x
                    self.hex_df['y'].iloc[df_old_id] = new_wgs_y
                    self.hex_df['geometry'].iloc[df_old_id] = new_wgs_polygon
